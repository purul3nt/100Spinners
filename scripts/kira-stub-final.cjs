...
(async () => {
  ...
  await page.addInitScript(() => {
    ...
    // CRITICAL: window.hacksaw.casino.X is where the bundle reads config
    window.hacksaw = {
      casino: {
    ...
    // The runtime API the integration uses
    ...
    console.log('[stub] hacksaw.casino.* config + hacksawCasino runtime injected');
  });
  ...
  page.on('console', msg => {
    ...
    if (/WIN-ERR|UNH-REJ|stub/.test(t)) console.log(`[${msg.type()}] ${t.slice(0, 250)}`);
  });
  page.on('pageerror', err => {
    errors.push(`ERR: ${err.message}`);
    if (err.stack) console.log('[ERRSTACK]', err.stack.slice(0, 500));
  });
  ...
  await page.screenshot({ path: `${OUT}/14-stub-final.png` });
